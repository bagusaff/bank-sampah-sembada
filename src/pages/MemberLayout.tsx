...
import { Leaf, LayoutDashboard, History, Wallet, Bell, Settings, LogOut, Menu, X, UserCircle } from "lucide-react";
	...
	return (
		<aside className="w-[260px] bg-white border-r border-slate-200/60 flex flex-col h-full shadow-[2px_0_24px_-12px_rgba(0,0,0,0.06)]">
			...
			<nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
				...
				{NAV_ITEMS.map((item) => (
					<NavItem key={item.to} {...item} badge={item.to === "/member/notifications" ? unreadCount : undefined} onClick={onClose} />
				))}
			</nav>
...
export default function MemberLayout() {
	...
	return (
		<div className="flex h-screen bg-slate-50 overflow-hidden">
			{/* ── Desktop sidebar ── */}
			{!isMobile ? <Sidebar onSignOut={signOut} memberName={memberName} unreadCount={unreadCount} /> : null}
			...
			{isMobile && sidebarOpen ? (
				<>
					...
					<div className="fixed inset-y-0 left-0 z-40 animate-in slide-in-from-left duration-200">
						<Sidebar onClose={closeSidebar} onSignOut={signOut} memberName={memberName} unreadCount={unreadCount} />
					</div>
				</>
			) : null}
			...
			<div className="flex-1 flex flex-col overflow-hidden min-w-0">
				...
				{!isMobile ? (
					<div className="flex items-center justify-between px-8 py-4 bg-white border-b border-slate-100">
						<div>
							<p className="text-xs font-black text-slate-400 uppercase tracking-widest">Selamat datang</p>
							<p className="text-base font-black text-slate-900 tracking-tight">{memberName}</p>
						</div>
						<div className="flex items-center gap-2">
